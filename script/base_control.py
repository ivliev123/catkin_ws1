...
class BaseControl:
    def __init__(self):
        ...
        self.baudrate = float( rospy.get_param('~baudrate', '57600') ) # baudrate
        self.odom_freq = float( rospy.get_param('~odom_freq', '50') ) # hz of odom pub
        self.wheelSep = float( rospy.get_param('~wheel_separation', '0.41') ) # unit: meter 
        self.wheelRad = float( rospy.get_param('~wheel_radius', '0.08') ) # unit: meter
    ...
    def timerOdomCB(self, event):
        ...
        try:           
            ...
            str_from_inRaw = str(data)
            str_from_inRaw=str_from_inRaw[0:-3]
		
            str_array_to_parse = str_from_inRaw.replace('.', '').split()

            rospy.loginfo(str_array_to_parse) 
            #rospy.loginfo(str_array_to_parse[0][1:])

            #WL = parseint(str_array_to_parse[0])
            #WR = parseint(str_array_to_parse[1])
            WR = float(str_array_to_parse[0][1:])/100
            WL = float(str_array_to_parse[1][1:])/100
...
#                print 'Error Value! header1: ' + str(int(data[0].encode('hex'),16)) + ', header2: ' + str(int(data[1].encode('hex'),16))          
	...
	#rospy.loginfo(WL)
        ...
        R_forward = 0 # 0: reverse, >0: forward  
        L_forward = 0 # 0: reverse, >0: forward       
        if self.WR_send < 0:
            R_forward = 1
	...
	rospy.loginfo( '%.2f' % WR)
	rospy.loginfo( '%.2f' % WR_send)

        R=struct.pack('<H',self.WR_send)
        L=struct.pack('<H',self.WL_send)

        D1=struct.pack('<H',R_forward)
        D2=struct.pack('<H',L_forward)
